<template>
  ...
  >
    <template #header>
      <span>{{ isZh ? '净资产趋势' : 'Net Assets Trend' }}</span>
    </template>
...
const actualData = computed(() =>
  dailyDataWithPrediction.value.map(day =>
    day.date <= todayStr ? day.netAssets : null
  )
)
...
const chartOption = computed(() => ({
  ...
  legend: {
    data: [
      isZh.value ? '累计净资产' : 'Cumulative Net Assets',
      isZh.value ? '预测净资产' : 'Predicted Net Assets',
    ],
    textStyle: {
  ...
  series: [
    {
      name: isZh.value ? '累计净资产' : 'Cumulative Net Assets',
      type: 'line',
      smooth: false,
    ...
    {
      name: isZh.value ? '预测净资产' : 'Predicted Net Assets',
      type: 'line',
      smooth: false,
