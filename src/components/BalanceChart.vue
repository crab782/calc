<template>
  <el-card shadow="never" v-if="dailyDataWithPrediction.length === 0" class="chart-card">
    <el-empty :description="t.chart.noData">
      <template #description>
        ...
        <p class="hint-text">
          {{ isZh ? '添加交易记录后将显示趋势图表' : 'Add records to see the trend chart' }}
        </p>
      </template>
    </el-empty>
  </el-card>
  ...
  >
    <template #header>
      <span>{{ isZh ? '结余趋势' : 'Balance Trend' }}</span>
    </template>
...
const chartOption = computed(() => ({
  tooltip: {
    ...
    formatter: (params: { axisValue: string; marker: string; seriesName: string; value: number | null; dataIndex: number }[]) => {
      ...
      result += `<div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid ${tooltipBorder.value};">`
      result += `<div style="font-size: 12px;">${isZh.value ? '当日收支' : 'Daily Flow'}</div>`
  ...
  legend: {
    data: [
      isZh.value ? '累计结余' : 'Cumulative Balance',
      isZh.value ? '预测结余' : 'Predicted Balance',
    ],
    textStyle: {
  ...
  series: [
    {
      name: isZh.value ? '累计结余' : 'Cumulative Balance',
      type: 'line',
      smooth: false,
    ...
    {
      name: isZh.value ? '预测结余' : 'Predicted Balance',
      type: 'line',
      smooth: false,
