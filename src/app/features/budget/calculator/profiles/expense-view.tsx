...
export function ExpenseView() {
  ...
  const handleCalculate = () => {
    const calc = calculateBudget('expense' as BudgetProfileType, records, sources, periodCount, currency);
    setResult(calc);
    message.success('支出计算完成');
  };

  const chartOption = useMemo(() => {
    if (!result) return {};
    return {
      title: { text: '支出预测', left: 'center' },
      tooltip: { trigger: 'axis' },
      xAxis: { type: 'category', data: result.labels },
      yAxis: { type: 'value' },
      series: [{ type: 'bar', data: result.data, itemStyle: { color: '#fa8c16' } }],
  ...
  const tableColumns = useMemo(() => [
    ...
    { title: '预计支出', dataIndex: 'amount', key: 'amount', render: (v: number) => `¥${v.toFixed(2)}` },
  ], []);
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }} size={24}>
      <ChartPanel title="支出预测" option={chartOption} />
      <TablePanel title="支出明细" dataSource={tableData as unknown as Record<string, unknown>[]} columns={tableColumns as unknown as Record<string, unknown>[]} />
    </Space>
  );
}
