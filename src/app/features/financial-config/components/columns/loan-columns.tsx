...
import type { FinancialSource, FinancialPeriod, InterestType } from '../../../../../types/record';
...
export const useLoanColumns = ({ onEdit, onDelete }: LoanColumnsProps) => {
  const { t } = useLanguage();
  ...
  return useMemo<TableColumnsType<FinancialSource>>(() => {
    ...
    const interestTypeLabel = (type: InterestType | undefined): string => {
      if (!type) return '—';
      const map: Record<InterestType, string> = {
        'equal-payment': t.financialConfig.equalPayment,
        'equal-principal': t.financialConfig.equalPrincipal,
        'interest-first': t.financialConfig.interestFirst,
      };
      return map[type];
    };
    ...
    return [
      ...
      {
        title: t.financialConfig.principal,
        dataIndex: 'principal',
        key: 'principal',
        width: 100,
        render: (value: number | undefined, record: FinancialSource) =>
          value !== undefined ? `${CURRENCY_SYMBOLS[record.currency] || ''}${value.toFixed(2)}` : '—',
      },
      {
        title: t.financialConfig.interestRate,
      ...
      {
        title: t.financialConfig.interestType,
        dataIndex: 'interestType',
        key: 'interestType',
        width: 90,
        render: (value: InterestType | undefined) => interestTypeLabel(value),
      },
      {
