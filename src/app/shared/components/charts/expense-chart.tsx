...
export function ExpenseChart() {
  ...
  const xAxisLabels = dailyData.map(day => {
  ...
  const actualData = dailyData.map(day =>
    day.date <= todayStr ? day.expense : null
  );

  const predictedData = dailyData.map(day =>
    day.date >= todayStr ? day.expense : null
  );
  ...
  const option = {
    tooltip: {
      ...
      formatter: (params: { axisValue: string; marker: string; seriesName: string; value: number | null; dataIndex: number }[]) => {
        ...
        const statusLabel = dayData.expense > 0
    ...
    legend: {
      data: [t.chart.expense, language === 'zh' ? '预测支出' : 'Predicted Expense'],
